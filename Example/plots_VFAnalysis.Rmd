...
df_all_mean <- round(df_all_mean*100,2)
rownames(df_all_mean)<- c("Automatic frames", "Automatic seconds", "Manual coding")

pander(df_all_mean)

png("Plots/table_visual_fields_all.png", height = 150*nrow(df_all_mean), width = 550*ncol(df_all_mean), res=300)
p<-tableGrob(df_all_mean)
grid.arrange(p)
dev.off()

png("Plots/table_visual_fields_seconds.png", height = 150*nrow(df_all_mean), width = 550*ncol(df_all_mean), res=300)
p<-tableGrob(df_all_mean[c("Automatic seconds", "Manual coding"),])
grid.arrange(p)
dev.off()
...
rownames(df_all_mean_LI)<- c("Automatic frames", "Automatic seconds", "Manual coding")

pander(df_all_mean_LI)

df_all_mean_LI2 <- df_all_mean_LI
colnames(df_all_mean_LI2) <- c("overall", "lateral", "frontal")
png("Plots/table_LI_all.png", height = 150*nrow(df_all_mean_LI2), width = 550*ncol(df_all_mean_LI2), res=300)
p<-tableGrob(df_all_mean_LI2)
grid.arrange(p)
dev.off()

png("Plots/table_LI_seconds.png", height = 150*nrow(df_all_mean_LI2), width = 550*ncol(df_all_mean_LI2), res=300)
p<-tableGrob(df_all_mean_LI2[c("Automatic seconds", "Manual coding"),])
grid.arrange(p)
dev.off()
...
ggplot(new_df_dlc[new_df_dlc$condition=="overall",], aes(x=seconds, y=LI))+
  ...
  theme(panel.grid.major.x = element_blank()) +
  theme(panel.grid.major.y = element_blank()) +
   labs(y="Left <-- Laterality Index [%] --> Right", x="time in seconds", title="Chick 1 LI through time - overall (precision: frames)")
if (print_pic==TRUE){dev.off()}
...
ggplot(new_df_dlc[new_df_dlc$condition=="frontal" | new_df_dlc$condition=="lateral",], aes(x=seconds, y=LI, color=condition))+
  ...
  theme(panel.grid.major.x = element_blank()) +
  theme(panel.grid.major.y = element_blank()) +
   labs(y="Left <-- Laterality Index [%] --> Right", x="time in seconds", title="Chick 1 LI through time - lateral and frontal (precision: frames)")
if (print_pic==TRUE){dev.off()}
```
...
ggplot(new_df_dlc_2[new_df_dlc_2$condition=="overall",], aes(x=seconds, y=LI))+
  ...
  theme(panel.grid.major.x = element_blank()) +
  theme(panel.grid.major.y = element_blank()) +
   labs(y="Left <-- Laterality Index [%] --> Right", x="time in seconds", title="Chick 1 LI through time - overall (precision: seconds)")
if (print_pic==TRUE){dev.off()}
```
...
ggplot(new_df_dlc_2[new_df_dlc_2$condition=="frontal" | new_df_dlc_2$condition=="lateral",], aes(x=seconds, y=LI, color=condition))+
  ...
  theme(panel.grid.major.x = element_blank()) +
  theme(panel.grid.major.y = element_blank()) +
   labs(y="Left <-- Laterality Index [%] --> Right", x="time in seconds", title="Chick 1 LI through time - lateral and frontal(precision: seconds)")
if (print_pic==TRUE){dev.off()}
```
...
ggplot(new_df_mc[new_df_mc$condition=="overall",], aes(x=seconds, y=LI))+
  ...
  theme(panel.grid.major.x = element_blank()) +
  theme(panel.grid.major.y = element_blank()) +
   labs(y=" Left <-- Laterality Index [%] --> Right", x="time in seconds", title="Chick 1 LI through time - overall (precision: seconds) with manual coding")
if (print_pic==TRUE){dev.off()}
```
...
ggplot(new_df_mc[new_df_mc$condition=="frontal" | new_df_mc$condition=="lateral",], aes(x=seconds, y=LI, color=condition))+
  ...
  theme(panel.grid.major.x = element_blank()) +
  theme(panel.grid.major.y = element_blank()) +
   labs(y=" Left <-- Laterality Index [%] --> Right", x="time in seconds", title="Chick 1 LI through time - lateral and frontal (precision: seconds) with manual coding")
if (print_pic==TRUE){dev.off()}
```
<br/>
...
ggplot(df_all[df_all$coding_type=="manual" | df_all$coding_type=="automatic_seconds",], aes(x=condition, y=LI, fill=coding_type)) + 
  ...
  labs(title= "Comparaison of Laterality Index computed automatically and manually", y=" Left <--   Laterality Index [%]  --> Right", x="") +
  scale_fill_discrete(name = "condition", labels = c("automatic", "manual")) 
if (print_pic==TRUE){dev.off()}
...
### Differences between manual and automatic coding

Are manual and automatic coding correlated? We use correlations methods such as *Pearson*'s and *Spearman*'s (non parametric) correlation test.

We compare only manual coding versus *VFA* output with seconds to have the same scale.

We gather data in a dataframe where:

 - column 1 is the *type* of visual field used: frontal, overall, lateral.
 - column 2 is the LI for manual coding
 - column 3 is the LI for *VFA* output gathered by seconds




```{r correlation , echo=FALSE, message=FALSE, warning=FALSE}
df_1 <- data.frame(LI_front = chick1mc$LI_frontal, LI_lat = chick1mc$LI_lateral, LI_overall = chick1mc$LI_overall)
df_compare <- gather(df_1, condition, LI_mc, LI_front:LI_overall)
df_2 <- data.frame(LI_front = chick1dlc_seconds$LI_frontal, LI_lat = chick1dlc_seconds$LI_lateral, LI_overall = chick1dlc_seconds$LI_overall)
df_2 <- gather(df_2, condition, LI_aut, LI_front:LI_overall)
df_compare$LI_aut <- df_2$LI_aut
```

Do our data follow a normal distribution?
```{r shapiro, echo=FALSE, message=FALSE, warning=FALSE}
shapiro.test(df_compare$LI_mc)
shapiro.test(df_compare$LI_aut)
# significantly different from a normal distribution
```
--> our data does **not** follow a normal distribution, so we rely on non-parametric test. 

(there are other test to do for other assumptions... As we only want to study correlations, it was not needed here)

We compared the distribution of points for *column 2 and 3*.

*Pearson correlation test for all (overall, frontal, lateral):*
```{r correlation pearson, echo=FALSE, message=FALSE, warning=FALSE}

cor.test( ~ LI_mc + LI_aut,
         data=df_compare,
         method = "pearson",
         continuity = FALSE,
         use="complete.obs",
...
*Spearman correlation test (overall, frontal, lateral):*
```{r correlation spearman, echo=FALSE, message=FALSE, warning=FALSE}
...
*Pearson correlation test for overall:*
```{r correlation pearson overall, echo=FALSE, message=FALSE, warning=FALSE}

cor.test( ~ LI_mc + LI_aut,
         data=df_compare[df_compare$condition=="LI_overall",],
         method = "pearson",
         continuity = FALSE,
         use="complete.obs",
         conf.level = 0.95)
...
*Spearman correlation test for overall:*
```{r correlation spearman overall, echo=FALSE, message=FALSE, warning=FALSE}
...
*Pearson correlation test for lateral:*
```{r correlation pearson lateral, echo=FALSE, message=FALSE, warning=FALSE}

cor.test( ~ LI_mc + LI_aut,
         data=df_compare[df_compare$condition=="LI_lat",],
         method = "pearson",
         continuity = FALSE,
         use="complete.obs",
         conf.level = 0.95)
...
*Spearman correlation test for lateral:*
```{r correlation spearman lateral, echo=FALSE, message=FALSE, warning=FALSE}
...
*Pearson correlation test for frontal:*
```{r correlation pearson frontal, echo=FALSE, message=FALSE, warning=FALSE}

cor.test( ~ LI_mc + LI_aut,
         data=df_compare[df_compare$condition=="LI_front",],
         method = "pearson",
         continuity = FALSE,
         use="complete.obs",
         conf.level = 0.95)
...
*Spearman correlation test for frontal:*
```{r correlation spearman frontal, echo=FALSE, message=FALSE, warning=FALSE}
...
--> Our data have a *high* and *positive* correlation: it means that the manual and automatic coding are very similar.

 The manual coding for lateral view was perfect, whereas the coding for frontal view is more different.


```{r stat test, echo=FALSE, message=FALSE, warning=FALSE}
t.test(x= df_compare$LI_mc, y = df_compare$LI_aut)
wilcox.test(x= df_compare$LI_mc, y = df_compare$LI_aut) # non parametric
```

Our two means are **not** significantly different.
